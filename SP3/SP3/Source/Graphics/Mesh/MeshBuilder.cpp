...
#include "MyMath.h"
#include "../../Graphics/LoadOBJ/LoadOBJ.h"
#include "../../Graphics/Mesh/LoadHmap.h"
/******************************************************************************/
/*!
\brief
...
*/
/******************************************************************************/
Mesh* MeshBuilder::GenerateQuad(const std::string &meshName, Color color, float length, int texCount)
{
	Vertex v;
	std::vector<Vertex> vertex_buffer_data;
	std::vector<GLuint> index_buffer_data;
	...
	v.color = color;
	v.normal.Set(0, 0, 1);
	v.texCoord.Set(1.0f * texCount, 0);
	vertex_buffer_data.push_back(v);
	v.pos.Set(0.5f * length, 0.5f * length,0);
	v.color = color;
	v.normal.Set(0, 0, 1);
	v.texCoord.Set(1.0f * texCount, 1.0f * texCount);
	vertex_buffer_data.push_back(v);
	v.pos.Set(-0.5f * length, 0.5f * length,0);
	v.color = color;
	v.normal.Set(0, 0, 1);
	v.texCoord.Set(0, 1.0f * texCount);
	vertex_buffer_data.push_back(v);
...
Mesh* MeshBuilder::GenerateSkyPlane(const std::string &meshName, Color color, int slices, float PlanetRadius, float AtmosphereRadius, float hTile, float vTile)
{
	...
	//Check whether number of slice is within 1 - 256
	...
	//find plane size
	float planeSize = 2.0 * (float)sqrt((AtmosphereRadius * AtmosphereRadius) - (PlanetRadius * PlanetRadius));
	...
	for (int z = 0; z <= slices; z++)
	{
		...
		{
			float xDis = (-0.5f * planeSize) + ((float)x * delta);
			float zDis = (-0.5f * planeSize) + ((float)z * delta);

			float xHeight = (xDis * xDis) / AtmosphereRadius;
			float zHeight = (zDis * zDis) / AtmosphereRadius;
